...
import { bookingApi, courtApi, reviewApi, ApiBooking, ApiCourt } from "../services/api";
...
const BookingsPage = () => {
  ...
  const [filter, setFilter] = useState<
    "ALL" | "UPCOMING" | "HISTORY" | "CANCELLED"
  >("ALL");
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const mapped: Booking[] = (bookingRes.bookings || []).map(
          (b: ApiBooking) => ({
            id: b._id,
            userId: b.userId,
            courtId: b.courtId,
  ...
  const filteredBookings = allBookings.filter((booking) => {
    ...
    switch (filter) {
      ...
      case "HISTORY":
        return (
          booking.status === BookingStatus.COMPLETED ||
          (!isUpcoming && booking.status !== BookingStatus.CANCELLED)
        );
      case "CANCELLED":
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4">
        <AnimatePresence>
          {loading ? (
            <div className="text-center py-12 bg-surface border border-neutral-800">
              <Loader2 className="animate-spin mx-auto w-8 h-8 text-gray-500 mb-4" />
                  ...
                  <div className="flex gap-4 items-start w-full md:w-auto">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-2 text-sm text-gray-400">
                        ...
                        <span className="flex items-center">
                          <Banknote size={14} className="mr-1" /> Rs.{" "}
                          {booking.totalPrice}
                        </span>
                        <span className="font-mono text-xs opacity-50 hidden md:inline">
                          #{booking.id}
                        </span>
                      </div>
                    </div>
                      ...
                      {booking.status === BookingStatus.COMPLETED && (
                        <>
                          <Button
                            ...
                            onClick={() => {
                              const receipt = `COURTSYNC - BOOKING RECEIPT\n================================\nBooking ID: #${booking.id.slice(-8).toUpperCase()}\nCourt:      ${booking.courtName}\nDate:       ${booking.date}\nSlots:      ${booking.slots.join(', ')}\nTotal:      Rs. ${booking.totalPrice}\n================================\nThank you for booking with CourtSync!`;
                              const blob = new Blob([receipt], { type: 'text/plain' });
                              const url = URL.createObjectURL(blob);
                              const a = document.createElement('a'); a.href = url; a.download = `receipt-${booking.id.slice(-8)}.txt`; a.click();
